...
number of forecasts for biodiversity is increasing, there is little evidence
showing that these forecasts work well at decadal time scales. Most biodiversity
forecasts are not evaluated to determine how well they predict future diversity,
fail to account for uncertainty, and do not use time-series data that captures
the actual dynamics being studied. In this study, we used best practices and
32-year time series to evaluate six different modeling approaches to predicting
changes in North American breeding birds' species richness. Hindcasts for each
method were evaluated annually for a decade at `r numbers$N_predict_sites` sites
distributed throughout the continental United States. While each model could
explain most of the variance in richness, none of them outperformed baseline
models that predicted constant richness at each site. In particular, we found no
evidence that current methods (such as species distribution models) can
successfully turn spatial data into useful temporal predictions about
biodiversity at decadal time-scales. To facilitate the rapid improvement of
biodiversity forecasts, we emphasize the value of specific best practices at
multiple stages of the forecasting pipeline, from the choice of predictor
variables through the choice of approach for evaluating model performance over
time.
